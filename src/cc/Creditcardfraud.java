...
public class Creditcardfraud extends Applet implements ActionListener {
    ...
    public void init(){
    	...
    	Label head=new Label("                                                                                               CREDIT CARD FRAUD DETECTION SYSTEM                                                               	                             ",Label.CENTER);
    	
    	 Font font = new Font("Serif", Font.ITALIC, 30);
    	head.setFont(font);
    	Label dataset=new Label("                                 DATASET SELECTED	                                                                             ",Label.CENTER);
    	
    	Label res=new Label("                                                   FRAUD DETECTED	          ",Label.CENTER);
    	  	
    	browses = new Label("      Browse DataSet:     ", Label.LEFT);
    	
    	concl = new Label("                                                    FRAUD TRANSACTIONS                             ", Label.LEFT);
		 ...
		 if(str.equals("   Browse   ")) {
			...
			//	System.out.println(" 1");
				...
				for(int i=0;i<=20;i++)
				{
					...
					{
					db.append("\n");
					db.append("______\t____\t______\t__\t______\t__\t_____\t___\t___\t____\t___\t____");
					db.append("\n");
					}
					db.append("\n"); System.out.println(" ");
				  ...
				  if(res[0]>=1)
					 {
					    ...
					    loc[l][m]=res[1];	
						result.append("In CC ID: "+data[i][0]+" - Usage Location Fraud is found with value - "+res[1]);
						result.append("\n");
						l++;m=0;
					 }	
				  initPop[i][1]=res[1];
				  ...
				  if(res[0]>=1)
					 {
					    ...
					    od[l][m]=res[1];	
						result.append("In CC ID: "+data[i][0]+" - CC OverDraft Fraud is found with value - "+res[1]);
						result.append("\n");
						l++;m=0;
					 }		
				  initPop[i][2]=res[1];
				  ...
				  if(res[0]>=1)
					 {
					    ...
					    bb[l][m]=res[1];	
						result.append("In CC ID: "+data[i][0]+" - CC Book Balance Fraud is found with value - "+res[1]);
						result.append("\n");
						l++;m=0;
					 }	
				  initPop[i][3]=res[1];
				  ...
				  if(res[0]>=1)
					 {
					    ...
					    ds[l][m]=res[1];	
						result.append("In CC ID: "+data[i][0]+" - CC Daily Spending Fraud is found with value - "+res[1]);
						result.append("\n");
						l++;m=0;
					 }	
				  initPop[i][4]=res[1];
			 ...
			 for(int i=1;i<=20;i++)
			 {
			    ...
			    con.append(" Critical Fraud Detected: ");con.append("\n");
				 ...
				 for(int i=0;i<=19;i++)
				 {
					   	...
					   	{
					   		con.append("\n");
					   	con.append(" Credit Card with ID "+finalresult[i][0]+" is detected as fraud with "+finalresult[i][1]+" occurance and its crical value is "+ finalresult[i][2]);
